...
from app.models import User, Tweet, Media, follows, likes
import os
import uuid
...
async def get_current_user(api_key: str = Header(None), db: AsyncSession = Depends(get_db)) -> User:
    if not api_key:
...
async def delete_tweet(
    ...
    result = await db.execute(
        delete(Tweet).where(and_(Tweet.id == tweet_id, Tweet.author_id == current_user.id))
    )
...
async def unlike_tweet(
    ...
    result = await db.execute(
        delete(likes).where(and_(likes.c.user_id == current_user.id, likes.c.tweet_id == tweet_id))
    )
    await db.commit()
    return {"result": True}
...
async def unfollow_user(
    ...
    result = await db.execute(
        delete(follows).where(and_(follows.c.follower_id == current_user.id, follows.c.following_id == user_id))
    )
    await db.commit()
    return {"result": True}
...
async def get_feed(
    ...
    result = await db.execute(
        select(Tweet).filter(Tweet.author_id.in_(following_ids))
